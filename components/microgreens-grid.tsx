...
export function MicrogreensGrid({ children }: MicrogreensGridProps) {
  ...
  return (
    <div className="flex-1 min-h-0 bg-background/50 overflow-hidden" ref={containerRef}>
      <ResponsiveGrid
        className="layout"
        layouts={layouts}
      ...
      >
        <div key="impact" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-auto bg-card">
              <div className="h-full">{children.impact}</div>
          </div>
        </div>

        <div key="environment" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          <div className="h-6 bg-muted/40 border-b border-border flex items-center px-2 cursor-grab active:cursor-grabbing drag-handle hover:bg-muted/60 transition-colors">
             <GripVertical className="w-3 h-3 text-muted-foreground mr-2" />
             <span className="text-[10px] font-mono text-muted-foreground uppercase">Environment</span>
          </div>
          <div className="flex-1 overflow-auto bg-card">
              <div className="h-full">{children.environment}</div>
          </div>
        </div>

        <div key="intelligence" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          <div className="h-6 bg-muted/40 border-b border-border flex items-center px-2 cursor-grab active:cursor-grabbing drag-handle hover:bg-muted/60 transition-colors">
             <GripVertical className="w-3 h-3 text-muted-foreground mr-2" />
             <span className="text-[10px] font-mono text-muted-foreground uppercase">Active Intelligence</span>
          </div>
          <div className="flex-1 overflow-auto bg-card">
              <div className="h-full">{children.intelligence}</div>
          </div>
        </div>
        
        <div key="library" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          <div className="h-6 bg-muted/40 border-b border-border flex items-center px-2 cursor-grab active:cursor-grabbing drag-handle hover:bg-muted/60 transition-colors">
             <GripVertical className="w-3 h-3 text-muted-foreground mr-2" />
             <span className="text-[10px] font-mono text-muted-foreground uppercase">Crop Library</span>
          </div>
          <div className="flex-1 overflow-auto bg-card">
              <div className="h-full">{children.library}</div>
          </div>
        </div>

        <div key="fsm" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          <div className="h-6 bg-muted/40 border-b border-border flex items-center px-2 cursor-grab active:cursor-grabbing drag-handle hover:bg-muted/60 transition-colors">
             <GripVertical className="w-3 h-3 text-muted-foreground mr-2" />
             <span className="text-[10px] font-mono text-muted-foreground uppercase">FSM Controller</span>
          </div>
          <div className="flex-1 overflow-hidden relative">
              <div className="absolute inset-0">{children.fsm}</div>
          </div>
        </div>

        <div key="garden" className="bg-card border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
          <div className="h-6 bg-muted/40 border-b border-border flex items-center px-2 cursor-grab active:cursor-grabbing drag-handle hover:bg-muted/60 transition-colors">
             <GripVertical className="w-3 h-3 text-muted-foreground mr-2" />
          ...
          <div className="flex-1 overflow-auto bg-card p-2">
              <div className="h-full">{children.garden}</div>
          </div>
        </div>
